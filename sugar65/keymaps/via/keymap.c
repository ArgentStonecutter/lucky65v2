...
#define FI_12_LEN_              12
const uint8_t f1_12_keycode_Win[FI_12_LEN_]=
{KC_MYCM, KC_WHOM, KC_MAIL, KC_CALC, KC_MSEL, KC_MSTP, KC_MPRV, KC_MPLY, KC_MNXT, KC_MUTE, KC_VOLD, KC_VOLU };
const uint8_t f1_12_keycode_MAC[FI_12_LEN_]=
{KC_BRIU, KC_BRID, _______, _______, _______, _______, KC_MPRV, KC_MPLY, KC_MNXT, KC_MUTE, KC_VOLD, KC_VOLU};

uint8_t f1_12_keycode_buf[FI_12_LEN_]={0};
...
void F1_12_Data_Init_Fun(void)
{
    if(confinfo.layer == _LB)
    {
        for(uint8_t i=0;i<FI_12_LEN_;i++)
        {
             f1_12_keycode_buf[i] = f1_12_keycode_Win[i];
        }
    }
    else
    {
        for(uint8_t i=0;i<FI_12_LEN_;i++)
        {
             f1_12_keycode_buf[i] = f1_12_keycode_MAC[i];
        }
    }
...
bool im_process_record_user(uint16_t keycode, keyrecord_t *record) {
    ...
    case KC_0:
    {
            if(confinfo.Num_To_F1 ==true){
                if (record->event.pressed) {
                        register_code16(f1_12_keycode_buf[keycode-KC_1 ]);
                    } else {
                        unregister_code16(f1_12_keycode_buf[keycode-KC_1 ]);
                    }
                    return false;
            }
    }
    ...
    case KC_MINS:
     {
            if(confinfo.Num_To_F1 ==true){
                if (record->event.pressed) {
                        register_code16(f1_12_keycode_buf[10]);
                    } else {
                        unregister_code16(f1_12_keycode_buf[10]);
                    }
                    return false;
            }
    }
    ...
    case KC_EQL:
    {
            if(confinfo.Num_To_F1 ==true){
                if (record->event.pressed) {
                        register_code16(f1_12_keycode_buf[11]);
                    } else {
                        unregister_code16(f1_12_keycode_buf[11]);
                    }
                    return false;
            }
    }
